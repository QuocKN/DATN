...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train linear classifiers on EfficientNet-B2 embeddings for drone spectrogram detection"
    )
    parser.add_argument(
        "--drone-root",
        type=str,
...
def main() -> None:
    ...
    drone_paths = choose_subset(
        collect_image_paths(Path(args.drone_root)),
        max_count=args.max_drone,
        seed=args.seed,
    )
    non_drone_paths = choose_subset(
        collect_image_paths(Path(args.non_drone_root)),
        max_count=args.max_non_drone,
        seed=args.seed + 1,
    )
    ...
    results = [
        train_and_eval(name, classifier, x_train, y_train, x_val, y_val)
