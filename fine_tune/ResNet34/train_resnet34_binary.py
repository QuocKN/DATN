...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained ResNet34 ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def build_optimizer(
    ...
    backbone_params = [p for name, p in model.named_parameters() if p.requires_grad and not name.startswith("fc.")]
    head_params = [p for name, p in model.named_parameters() if p.requires_grad and name.startswith("fc.")]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune ResNet34 for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/ResNet34/resnet34_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_resnet34_binary.pt"
    history = []
