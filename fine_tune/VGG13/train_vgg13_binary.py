...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained VGG13-BN ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def build_optimizer(
    ...
    backbone_params = [
        p for name, p in model.named_parameters() if p.requires_grad and not name.startswith("classifier.6.")
    ]
    head_params = [p for name, p in model.named_parameters() if p.requires_grad and name.startswith("classifier.6.")]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune VGG13-BN for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/VGG13/vgg13_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_vgg13_binary.pt"
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improvement > args.early_stop_min_delta:
            ...
            torch.save(
                {
                    "model_name": "vgg13_bn_binary_finetuned",
                    "backbone_name": "vgg13_bn",
