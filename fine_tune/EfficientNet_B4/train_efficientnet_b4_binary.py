...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained EfficientNet-B4 ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune EfficientNet-B4 for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/EfficientNet_B4/efficientnet_b4_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_efficientnet_b4_binary.pt"
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improvement > args.early_stop_min_delta:
            ...
            torch.save(
                {
                    "model_name": "efficientnet_b4_binary_finetuned",
