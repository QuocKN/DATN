...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained VGG11-BN ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune VGG11-BN for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/VGG11/vgg11_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_vgg11_binary.pt"
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improvement > args.early_stop_min_delta:
            ...
            torch.save(
                {
                    "model_name": "vgg11_bn_binary_finetuned",
                    "backbone_name": "vgg11_bn",
