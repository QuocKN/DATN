...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained GoogLeNet ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune GoogLeNet for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/GoogLeNet/googlenet_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_googlenet_binary.pt"
    history = []
