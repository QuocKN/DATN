...
OUTPUT_JSON = "fine_tune/OpenCLIP/report/Tu_thu/2toan_results.json"
OUTPUT_CHART = "fine_tune/OpenCLIP/report/Tu_thu/2toan_results_chart.png"
IMAGE_SIZE = 224
DEVICE = "cuda:0"
BATCH_SIZE = 64
NUM_WORKERS = 4
...
def build_model(checkpoint: dict, device: torch.device) -> tuple[nn.Module, List[str], dict, int, List[float], List[float], str, str]:
    openclip_model = checkpoint.get("openclip_model", "ViT-B-16")
    openclip_pretrained = checkpoint.get("openclip_pretrained", "openai")
    ...
    backbone = open_clip.create_model(model_name=openclip_model, pretrained=openclip_pretrained).to(device)
    feature_dim = infer_feature_dim(backbone=backbone, image_size=image_size, device=device)
    model = OpenCLIPClassifier(backbone=backbone, feature_dim=feature_dim, num_classes=len(class_names)).to(device)
    model.load_state_dict(checkpoint["state_dict"], strict=True)
    model.eval()

    return model, class_names, class_to_idx, image_size, mean, std, openclip_model, openclip_pretrained
...
def main() -> None:
    ...
    model, class_names, class_to_idx, image_size, mean, std, openclip_model, openclip_pretrained = build_model(checkpoint, device)
    ...
    summary = {
        "method": "openclip_finetuned_classifier",
        "model_name": checkpoint.get("model_name", "openclip_vitb16_binary_finetuned"),
