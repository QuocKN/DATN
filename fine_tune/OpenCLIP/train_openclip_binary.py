...
try:
    ...
    raise RuntimeError("Missing dependency 'open_clip_torch'. Install it first: pip install open_clip_torch") from exc
...
def load_openclip_backbone(model_name: str, pretrained: str, device: torch.device) -> nn.Module:
    try:
        backbone = open_clip.create_model(model_name=model_name, pretrained=pretrained)
    except Exception as exc:
        raise RuntimeError(
            "Could not load OpenCLIP model. Ensure internet access for first download and valid model/pretrained tags."
        ) from exc
    backbone.to(device)
    return backbone
...
def infer_feature_dim(backbone: nn.Module, image_size: int, device: torch.device) -> int:
    ...
    features = backbone.encode_image(dummy)
    if features.ndim != 2:
...
def unfreeze_last_n_blocks(backbone: nn.Module, train_last_n_blocks: int) -> int:
    if train_last_n_blocks <= 0:
        return 0

    blocks = getattr(getattr(backbone, "visual", None), "transformer", None)
    if blocks is None or not hasattr(blocks, "resblocks"):
        raise ValueError("OpenCLIP backbone does not expose visual.transformer.resblocks")

    resblocks = blocks.resblocks
    total_blocks = len(resblocks)
    n = min(train_last_n_blocks, total_blocks)
...
def build_model(
    ...
    device: torch.device,
) -> tuple[OpenCLIPClassifier, int]:
    backbone = load_openclip_backbone(model_name=model_name, pretrained=pretrained, device=device)
    feature_dim = infer_feature_dim(backbone=backbone, image_size=image_size, device=device)

    unfrozen_blocks = 0
    if train_last_n_blocks > 0:
        freeze_all_backbone_params(backbone)
        unfrozen_blocks = unfreeze_last_n_blocks(backbone, train_last_n_blocks)
    elif freeze_backbone:
        freeze_all_backbone_params(backbone)

    model = OpenCLIPClassifier(backbone=backbone, feature_dim=feature_dim, num_classes=len(CLASS_NAMES))
    model.to(device)
...
def build_optimizer(model: OpenCLIPClassifier, backbone_lr: float, head_lr: float, weight_decay: float) -> torch.optim.Optimizer:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune OpenCLIP ViT-B/16 for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/OpenCLIP/openclip_binary_runs")
    parser.add_argument("--openclip-model", type=str, default=OPENCLIP_MODEL)
    parser.add_argument("--openclip-pretrained", type=str, default=OPENCLIP_PRETRAINED)
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Freeze backbone then unfreeze the last N visual transformer blocks (overrides --freeze-backbone when > 0).",
    )
...
def main() -> None:
    ...
    print(f"OpenCLIP model: {args.openclip_model} | pretrained: {args.openclip_pretrained}")
    ...
    if args.train_last_n_blocks > 0:
        print(f"Unfrozen last OpenCLIP blocks: {unfrozen_blocks}")
    else:
    ...
    best_path = out_dir / f"balanced_openclip_binary.pt"
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improvement > args.early_stop_min_delta:
            ...
            torch.save(
                {
                    "model_name": "openclip_vitb16_binary_finetuned",
                    ...
                    "image_size": args.image_size,
                    "image_mean": OPENCLIP_MEAN,
                    "image_std": OPENCLIP_STD,
                    "args": vars(args),
    ...
    summary_path = out_dir / f"balanced_summary.json"
