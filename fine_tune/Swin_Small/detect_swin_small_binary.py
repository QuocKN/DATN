...
def build_model(checkpoint: dict, device: torch.device) -> tuple[nn.Module, List[str], dict, int, str]:
    ...
    model.eval()
    return model, class_names, class_to_idx, image_size, backbone_name
...
def visualize_detection_results(
    ...
    fig.suptitle(
        f"{drone_name}\n\n"
        f"Fine-tuned Swin V2 Small Detection Results | Model: {model_name or 'unknown'} | "
...
def main() -> None:
    ...
    model, class_names, class_to_idx, image_size, backbone_name = build_model(checkpoint, device)
    ...
    summary = {
        ...
        "model_name": checkpoint.get("model_name", "swin_v2_s_binary_finetuned"),
