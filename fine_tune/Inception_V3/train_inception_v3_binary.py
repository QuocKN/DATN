...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained Inception V3 ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune Inception V3 for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/Inception_V3/inception_v3_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_inception_v3_binary.pt"
    history = []
