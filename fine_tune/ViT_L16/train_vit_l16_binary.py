...
def build_model(freeze_backbone: bool) -> nn.Module:
    try:
        ...
        raise RuntimeError(
            "Could not load pretrained ViT-L/16 ImageNet weights; refusing to train from random initialization."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune ViT-L/16 for binary drone/non-drone spectrogram detection")
    ...
    parser.add_argument("--out-dir", type=str, default="fine_tune/ViT_L16/vit_l16_binary_runs")
...
def main() -> None:
    ...
    best_path = out_dir / "balanced_vit_l16_binary.pt"
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improvement > args.early_stop_min_delta:
            ...
            torch.save(
                {
                    "model_name": "vit_l16_binary_finetuned",
