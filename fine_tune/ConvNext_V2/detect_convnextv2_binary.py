...
def build_model(checkpoint: dict, device: torch.device) -> tuple[nn.Module, List[str], dict, int, List[float], List[float], str]:
    ...
    model = ConvNextV2BinaryClassifier(backbone=backbone, feature_dim=feature_dim, num_classes=len(class_names)).to(device)
    model.load_state_dict(checkpoint["state_dict"], strict=True)
    model.eval()

    return model, class_names, class_to_idx, image_size, mean, std, convnextv2_model
...
def visualize_detection_results(
    ...
    fig.suptitle(
        f"{drone_name}\n\n"
        f"Fine-tuned ConvNeXtV2 Detection Results | Model: {model_name or 'unknown'} | "
...
def main() -> None:
    ...
    model, class_names, class_to_idx, image_size, mean, std, convnextv2_model = build_model(checkpoint, device)
    ...
    summary = {
        "method": "convnextv2_finetuned_classifier",
        "model_name": checkpoint.get("model_name", "convnextv2_base_binary_finetuned"),
        "convnextv2_model": convnextv2_model,
