...
Chương này trình bày thiết kế thực nghiệm và phân tích kết quả của hệ thống phát hiện drone dựa trên tín hiệu RF. Các thí nghiệm được xây dựng nhằm đánh giá hai chiến lược sử dụng mô hình tiền huấn luyện là \textit{Feature Extraction-based Classification} và \textit{fine-tuning}, đồng thời khảo sát khả năng chuyển giao của sáu backbone thị giác gồm VGG13-BN, ResNet50, EfficientNet-B2, ConvNeXtV2, Swin Transformer và DINOv2 sang miền ảnh spectrogram. Ngoài phép đánh giá trên tập kiểm thử nội miền được xây dựng từ mẫu \textit{Drone} của DroneDetect v2 và mẫu \textit{Non-drone} tự thu/bổ sung, các mô hình còn được kiểm tra trên hai tập dữ liệu RF tự thu bằng bladeRF tại băng tần 2,4~GHz và các bộ dữ liệu ngoài miền gồm RFUAV, CageDroneRF, DRFF-R1 và DRFF-R2. Cách tổ chức này cho phép xem xét đồng thời hiệu năng trong miền dữ liệu chuẩn, mức độ tổng quát hóa sang điều kiện thu mới và ảnh hưởng của chất lượng spectrogram đến kết quả nhận dạng.
...
Với backbone $m$, mức cải thiện accuracy của fine-tuning so với \textit{Feature Extraction-based Classification} được tính bởi
\begin{equation}
...
Swin Transformer, cụ thể là biến thể Swin V2 Small trong thí nghiệm, là kiến trúc Transformer thị giác phân cấp. Mô hình sử dụng self-attention theo các cửa sổ cục bộ và cơ chế dịch cửa sổ giữa các tầng để trao đổi thông tin không gian. Việc đưa Swin Transformer vào thực nghiệm giúp kiểm tra khả năng chuyển giao của một backbone Transformer có cấu trúc phân cấp sang ảnh spectrogram RF.
...
Trong thí nghiệm \textit{Feature Extraction-based Classification}, ảnh spectrogram được đưa qua backbone đã đóng băng để trích xuất embedding. Các embedding của tập huấn luyện được dùng để huấn luyện SVM, trong khi embedding của tập kiểm thử chỉ được sử dụng để dự đoán. Kết quả cần được tổng hợp theo Bảng~\ref{tab:feature_extraction_internal}.

\begin{table}[H]
    \centering
    \caption{Kết quả Feature Extraction-based Classification trên tập kiểm thử nội miền}
...
Bảng~\ref{tab:fe_ft_comparison} được sử dụng để so sánh trực tiếp hai chiến lược trên cùng backbone. Cột chênh lệch phải được tính từ các giá trị đã điền trong hai bảng trước, không nhập bằng ước lượng.

\begin{table}[H]
    \centering
    \caption{So sánh Feature Extraction-based Classification và fine-tuning trên tập kiểm thử nội miền}
    \label{tab:fe_ft_comparison}
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|p{0.24\textwidth}|c|c|c|c|}
        \hline
        \textbf{Backbone} & $\mathbf{Acc_{FE}}$ \textbf{(\%)} & $\mathbf{Acc_{FT}}$ \textbf{(\%)} & $\boldsymbol{\Delta Acc}$ & $\boldsymbol{\Delta F1}$ \\ \hline
        VGG13-BN & -- & -- & [TÍNH] & [TÍNH] \\ \hline
        ResNet50 & -- & -- & [TÍNH] & [TÍNH] \\ \hline
        EfficientNet-B2 & -- & -- & [TÍNH] & [TÍNH] \\ \hline
        ConvNeXtV2 & -- & -- & [TÍNH] & [TÍNH] \\ \hline
        Swin Transformer & -- & -- & [TÍNH] & [TÍNH] \\ \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.22\textwidth}|c|c|c|c|c|}
        ...
        Swin Transformer & 1,0 & 2 & 1,5 & 3 & 1,25 \\ \hline
...
Swin Transformer chỉ đạt trung bình $1,25\%$, tương ứng 5 trên 400 ảnh đúng. Kết quả này cho thấy mô hình gần như dự đoán phần lớn mẫu drone tự thu thành \textit{Non-drone}. Dù checkpoint của Swin Transformer có kết quả nội miền cao, tỷ lệ nhận diện trên Drone1 và Drone2 giảm rất mạnh, cho thấy mô hình chịu ảnh hưởng lớn của domain shift trong kịch bản tự thu.

ConvNeXtV2 đạt trung bình $55,25\%$, tương ứng 221 trên 400 ảnh đúng và chỉ cao hơn một mức nhận diện $50\%$ là 5,25 điểm phần trăm. ResNet50 đạt trung bình $18,5\%$, tương ứng 74 trên 400 ảnh đúng và bỏ sót 326 ảnh. Swin Transformer có kết quả thấp nhất với 395 ảnh bị bỏ sót. Số liệu cho thấy các backbone này tổng quát hóa kém hơn rõ rệt trong kịch bản tự thu, dù nguyên nhân cần được đối chiếu với kết quả nội miền và ma trận nhầm lẫn.

Kết quả cũng cho thấy không thể kết luận Transformer luôn tốt hơn CNN. DINOv2 duy trì tỷ lệ nhận diện cao, nhưng Swin Transformer suy giảm rất mạnh trên cùng kịch bản tự thu. Ngược lại, VGG13-BN và EfficientNet-B2 là CNN nhưng đạt tỷ lệ trung bình cao, trong khi ConvNeXtV2 và ResNet50 cũng thuộc nhóm CNN nhưng có kết quả thấp hơn rõ rệt. Vì vậy, khả năng tổng quát hóa phụ thuộc vào từng backbone, trọng số tiền huấn luyện và quá trình fine-tuning, không chỉ phụ thuộc tên nhóm kiến trúc.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.24\textwidth}|c|c|c|}
        ...
        Swin Transformer & -- & 1,25 & [TÍNH] \\ \hline
...
Swin Transformer đạt $1,0\%$ trên Drone1 và $1,5\%$ trên Drone2. Mô hình chỉ nhận diện đúng 2 ảnh ở Drone1 và 3 ảnh ở Drone2. Chênh lệch giữa hai tập chỉ 0,5 điểm phần trăm, nhưng đây không phải dấu hiệu ổn định tích cực vì mức hiệu năng tuyệt đối rất thấp.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.27\textwidth}|c|c|c|}
        ...
        Swin Transformer & 1,25 & 0,5 & 395 \\ \hline
        ConvNeXtV2 & 55,25 & 11,5 & 179 \\ \hline
        ResNet50 & 18,5 & 4,0 & 326 \\ \hline
    \end{tabular}
\end{table}

Bảng~\ref{tab:real_stability} cho thấy VGG13-BN có tỷ lệ trung bình cao nhất, còn EfficientNet-B2 có độ chênh thấp nhất trong nhóm mô hình hiệu năng cao. DINOv2 có trung bình thấp hơn VGG13-BN 1,5 điểm phần trăm và bỏ sót nhiều hơn 7 ảnh. ConvNeXtV2, ResNet50 và Swin Transformer có số lượng bỏ sót lần lượt là 179, 326 và 395, cho thấy hạn chế rõ rệt trong kịch bản thực tế.
...
Chênh lệch giữa Drone1 và Drone2 trong Bảng~\ref{tab:real_recognition_results} cung cấp dấu hiệu rằng điều kiện dữ liệu ảnh hưởng khác nhau đến từng backbone. DINOv2 thay đổi 8 điểm phần trăm và ConvNeXtV2 thay đổi 11,5 điểm, trong khi EfficientNet-B2 chỉ thay đổi 1 điểm. Swin Transformer chỉ chênh 0,5 điểm phần trăm nhưng nằm ở mức nhận diện rất thấp, nên cần diễn giải cùng hiệu năng tuyệt đối. Tuy nhiên, chưa thể quy toàn bộ chênh lệch này cho chất lượng spectrogram nếu chưa đối chiếu SNR và artifact của hai tập. Điều kiện thu, loại tín hiệu và phân bố thời gian cũng có thể góp phần.
...
Trên hai tập tự thu, VGG13-BN có tỷ lệ trung bình cao nhất là $96,5\%$, EfficientNet-B2 đạt $96,0\%$ và DINOv2 đạt $95,0\%$. EfficientNet-B2 có độ chênh giữa hai tập thấp nhất trong nhóm mô hình hiệu năng cao, trong khi VGG13-BN đạt số mẫu đúng cao nhất. Swin Transformer chỉ đạt $1,25\%$, cho thấy kết quả nội miền cao không bảo đảm khả năng chuyển miền sang dữ liệu tự thu.

ConvNeXtV2, ResNet50 và Swin Transformer chỉ đạt trung bình lần lượt $55,25\%$, $18,5\%$ và $1,25\%$. Chênh lệch lớn giữa các backbone trên cùng dữ liệu cho thấy khả năng tổng quát hóa phụ thuộc vào biểu diễn được học và quá trình tối ưu của từng mô hình. Độ ổn định cũng phải được xem xét cùng mức hiệu năng tuyệt đối. ResNet50 và Swin Transformer có mức chênh thấp giữa Drone1 và Drone2, nhưng mức trung bình thấp nên không thể xem đây là sự ổn định có ý nghĩa tích cực.

Kết quả không cung cấp căn cứ để kết luận Transformer luôn tốt hơn CNN. VGG13-BN và EfficientNet-B2 là CNN nhưng đạt tỷ lệ trung bình cao hơn DINOv2 một chút và cao hơn Swin Transformer rất rõ rệt trong hai tập tự thu. Tuy nhiên, DINOv2 cũng duy trì kết quả cao hơn nhiều so với ConvNeXtV2 và ResNet50. Vì các mô hình còn khác nhau về dữ liệu tiền huấn luyện, quy mô và cách tổ chức đặc trưng, nhận xét phù hợp chỉ nên được đưa ra ở mức từng backbone.
