...
def save_spectrogram_chunks(
    ...
    hop_length=128,
    max_frames=8000,
    output_dir="spectrogram_chunks",
):
    """
    Cắt file IQ thành các đoạn nhỏ (chunk_ms), chuyển sang spectrogram và lưu từng ảnh vào output_dir.
    """
    ...
    for i in range(n_chunks):
        ...
        if x.size < n_fft:
            print(f"Chunk {i} quá ngắn, bỏ qua.")
            continue
...
if __name__ == "__main__":
    ...
    output_dir = r"C:\Users\DiepHM\Documents\AI\RF_Processing\CLEAN\PHA_HO\spec_chunks"  # Thư mục lưu ảnh

    Fs = 60e6
    fc = 2.4375e9
    # Điều chỉnh các tham số này theo ý bạn:
    start_time_ms = 5
    end_time_ms = 10
    chunk_ms = 5

    save_spectrogram_chunks(
        file_path=file_path,
        Fs=Fs,
        fc=fc,
