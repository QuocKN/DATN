...
def get_spectrogram_physical(
    ...
    """Physical spectrogram: PSD in dB/Hz.

    Formula: PSD = |FFT(x*w)|^2 / (Fs * sum(w^2))
    """
...
def plot_spectrogram(spec, time_axis, freq_axis, save_path=None):
    ...
    plt.title("Spectrogram (Physical PSD)")
...
def save_spectrogram_chunks_physical(
    file_path,
    Fs,
    fc,
    start_time_ms,
    ...
    hop_length=128,
    max_frames=8000,
    output_dir="spectrogram_chunks_physical",
):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i in range(n_chunks):
        ...
        spec, t, f = get_spectrogram_physical(
            x=x,
            Fs=Fs,
            fc=fc,
            n_fft=n_fft,
...
if __name__ == "__main__":
    ...
    output_dir = r"C:\Users\DiepHM\Documents\AI\RF_Processing\CLEAN\PHA_HO\spec_chunks_physical"
    ...
    save_spectrogram_chunks_physical(
        file_path=file_path,
        Fs=Fs,
        fc=fc,
